...
def main():
    for f_i in range(3):
        ...
        transductive_data_dir = f"/home/swang/xuhw/research-projects/data/drugbank_for_ddi/inductive/fold{f_i}/"
        save_dir = f"/home/swang/xuhw/research-projects/data/Pisces/drugbank_ind/fold{f_i}/"
        ...
        train_randoma_dir = "{}{}".format(save_dir, 'train.randoma')
        train_randomb_dir = "{}{}".format(save_dir, 'train.randomb')
        train_randompair_dir = "{}{}".format(save_dir, 'train.randompair')
        valid_randoma_dir = "{}{}".format(save_dir, 'valid.randoma')
        valid_randomb_dir = "{}{}".format(save_dir, 'valid.randomb')
        valid_randompair_dir = "{}{}".format(save_dir, 'valid.randompair')
        ...
        valid_len = len(valid_csv)
        drug_n = len(drug_id_dict)

        random.seed(f_i)
        train_random_pairs = []
        while len(train_random_pairs) < train_len:
            a, b = random.sample(range(drug_n), 2)
            if a != b:
                train_random_pairs.append((a, b))
        valid_random_pairs = []
        while len(valid_random_pairs) < valid_len:
            a, b = random.sample(range(drug_n), 2)
            if a != b:
                valid_random_pairs.append((a, b))

        with open(train_randoma_dir, 'w') as tra_w, open(train_randomb_dir, 'w') as trb_w, \
            open(train_randompair_dir, 'w') as trp_w:
            for a_id, b_id in train_random_pairs:
                a = id2dict[str(a_id)]
                b = id2dict[str(b_id)]
                tra_w.write(all_drug_dict[a] + '\n')
                trb_w.write(all_drug_dict[b] + '\n')
                trp_w.write(drug_id_dict[a] + '\t' + drug_id_dict[b] + '\n')
            tra_w.close()
            trb_w.close()
            trp_w.close()
        
        with open(valid_randoma_dir, 'w') as vra_w, open(valid_randomb_dir, 'w') as vrb_w, \
            open(valid_randompair_dir, 'w') as vrp_w:
            for a_id, b_id in valid_random_pairs:
                a = id2dict[str(a_id)]
                b = id2dict[str(b_id)]
                vra_w.write(all_drug_dict[a] + '\n')
                vrb_w.write(all_drug_dict[b] + '\n')
                vrp_w.write(drug_id_dict[a] + '\t' + drug_id_dict[b] + '\n')
