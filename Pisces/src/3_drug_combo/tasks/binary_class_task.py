...
class BinaryClassTask(FairseqTask):
    ...
    def __init__(self, cfg: BinaryClassConfig, data_dictionary, label_dictionary, cell_dictionary, drug_id_dictionary):
        super().__init__(cfg)
    ...
    def setup_task(cls, cfg: BinaryClassConfig, **kwargs):
        ...
        drug_id_dict = cls.load_dictionary(os.path.join(cfg.data, 'pairab', 'dict.txt'))
        logger.info(
        ...
        return cls(cfg, data_dict, label_dict, cell_dict, drug_id_dict)
    ...
    def load_dataset_src_tgt(self, split):
        ...
        drug_c_prefix = self.get_path("", '{}.{}-{}.{}'.format(split, self.drug_a, self.drug_c, self.drug_c))
        ...
        if not MMapIndexedDataset.exists(drug_c_prefix):
            raise FileNotFoundError("SMILES data {} not found.".format(drug_c_prefix))
        if not MolMMapIndexedDataset.exists(drug_c_prefix):
            raise FileNotFoundError("PyG data {} not found.".format(drug_c_prefix))
        ...
        drug_c_dataset = make_dataset(drug_c_prefix, impl=dataset_impl)
        assert drug_c_dataset is not None
        ...
        drug_c_dataset = AppendTokenDataset(
            TruncateDataset(
                StripTokenDatasetSizes(drug_c_dataset, self.target_dictionary.eos()),
                self._max_positions - 1,
            ),
            self.target_dictionary.eos(),
        )
        ...
        drug_c_dataset_graph = make_graph_dataset(drug_c_prefix, impl=dataset_impl)
        assert drug_c_dataset_graph is not None
        drug_c_dataset_graph = Tensor2Data(drug_c_dataset_graph)
        ...
        dataset = {
            ...
            "drug_c_seq": {
                "src_tokens": LeftPadDataset(drug_c_dataset, pad_idx=self.target_dictionary.pad()),
                "src_lengths": NumelDataset(drug_c_dataset),
            },
            "drug_a_graph": {"graph": src_dataset_graph,},
            "drug_b_graph": {"graph": tgt_dataset_graph,},
            "drug_c_graph": {"graph": drug_c_dataset_graph,},
        ...
        prefix_pairab = os.path.join(self.cfg.data, 'pairab', split)
        pairab_dataset = make_dataset(prefix_pairab, impl=dataset_impl)
        assert pairab_dataset is not None
        prefix_pairac = os.path.join(self.cfg.data, 'pairac', split)
        pairac_dataset = make_dataset(prefix_pairac, impl=dataset_impl)
        assert pairac_dataset is not None
        prefix_pairbc = os.path.join(self.cfg.data, 'pairbc', split)
        pairbc_dataset = make_dataset(prefix_pairbc, impl=dataset_impl)
        assert pairbc_dataset is not None

        dataset.update(
            pairab=OffsetTokensDataset(
                StripTokenDataset(pairab_dataset, id_to_strip=self.drug_id_dictionary.eos()),
                offset=-self.drug_id_dictionary.nspecial,
            )
        )
        dataset.update(
            pairac=OffsetTokensDataset(
                StripTokenDataset(pairac_dataset, id_to_strip=self.drug_id_dictionary.eos()),
                offset=-self.drug_id_dictionary.nspecial,
            )
        )
        dataset.update(
            pairbc=OffsetTokensDataset(
                StripTokenDataset(pairbc_dataset, id_to_strip=self.drug_id_dictionary.eos()),
                offset=-self.drug_id_dictionary.nspecial,
            )
        )
