...
class HeadFastTopK(nn.Module):
    ...
    def get_pred_batch(self, a, b, cells):
        if len(a.shape) == 2:
            a, b = a.unsqueeze(1), b.unsqueeze(1)
        cells_batch = cells.repeat(1, a.shape[1], 1)
        x = self.classifier_1(torch.cat([a, b, cells_batch], dim=-1))
    ...
    def forward(self, drug_a, dv_drug_a, drug_b, dv_drug_b, cells, pair=None, labels=None):
        ...
        pred, all_combo_emb = self.get_pred_batch(a_exp_i, b_exp_i, cell_embeddings)
