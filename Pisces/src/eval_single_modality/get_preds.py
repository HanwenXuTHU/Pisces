...
def validate(
    ...
    for i, sample in enumerate(tqdm(progress)):
        ...
        with torch.no_grad():
            ...
            cell_list.append(sample['cell'].cpu().numpy().reshape(-1))
    ...
    print(f"bacc: {bacc:.4f}, auc_prc: {auc_prc:.4f}, f1_score: {f1_score:.4f}, kappa: {kappa:.4f}")
    ...
    with open(file_name, 'w') as f:
        ...
        f.write(f"bacc: {bacc:.4f}, auc_prc: {auc_prc:.4f}, f1_score: {f1_score:.4f}, kappa: {kappa:.4f}")
    f = open(os.path.join(save_dir, 'pred.csv'), 'w')
    f.write('cell,drug1,drug2,pred,label\n')
    for i in range(len(predic)):
        f.write(f'{cell[i]},{pair[i][0]},{pair[i][1]},{predic[i]},{y_test[i]}\n')
    f.close()
