...
class HeadsClassify(nn.Module):
    ...
    def __init__(self,
                 ...
                 raw_data_path='',
                 drug_target_path='',
                 out_modal='SMILES+Graph+3D+Side_effect+Drug_Sensitivity_(NCI60)+Text+Drug_Ontology+Drug_target',
                 fp16=True,
                 wta_linear=True):
        ...
        self.topk = topk
        self.fp16 = fp16
        if 'Drug_Sensitivity_NCI60' in out_modal:
            out_modal = out_modal.replace('Drug_Sensitivity_NCI60', 'Drug_Sensitivity_(NCI60)')
        self.out_modal = out_modal.replace('_', ' ').split('+')
    ...
    def set_nn(self, input_dim, dv_input_dim, inner_dim, pooler_dropout, wta_linear, drug_n):
        ...
        self.sider_encoder = None
        self.sider_unk_emb = None
        self.nci60_encoder = None
        self.nci60_unk_emb = None
        self.dron_encoder = None
        self.dron_unk_emb = None
        self.text_encoder = None
        self.drug_3d_encoder = None
    ...
    def get_pred_batch(self, a, b, gene_exps):
        if len(a.shape) == 2:
            a, b = a.unsqueeze(1), b.unsqueeze(1)
        gene_exps_batch = gene_exps.repeat(1, a.shape[1], 1)
        x_hat = self.classifier_1(torch.cat([a, b, gene_exps_batch], dim=-1))
    ...
    def expand_ab(self, a, b, a_mask=None, b_mask=None):
        ...
        if a_mask is not None:
            a_exp_mask = torch.gather(a_mask.reshape([a.shape[0], -1]), 1, ind_a[:, :, 0])
            b_exp_mask = torch.gather(b_mask.reshape([a.shape[0], -1]), 1, ind_b[:, :, 0])
            return a_exp, b_exp, a_exp_mask, b_exp_mask
        return a_exp, b_exp

    def forward(self, drug_a, dv_drug_a, drug_b, dv_drug_b, gene_exps, pair=None, labels=None):
        gene_exps_neighbors = []
        for hop in range(self.n_hop):
            gene_exps_neighbors.append(torch.LongTensor([self.gene_exp_neighbor_set[c][hop] \
                                                       for c in gene_exps.squeeze(1).cpu().numpy().tolist()]).to(gene_exps.device)) 
        ...
        pred, all_combo_emb = self.get_pred_batch(a_exp_i, b_exp_i, gene_exp_embeddings)
        if len(pred.shape) == 1: pred, all_combo_emb = pred.unsqueeze(-1), all_combo_emb.unsqueeze(1)
        out = self.wta_layer(pred, mask)
        combo_idxes = np.random.choice(range(all_combo_emb.size(1)), size=2, replace=True, \
