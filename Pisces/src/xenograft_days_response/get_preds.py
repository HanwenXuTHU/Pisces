...
def validate(
    ...
    f = open(os.path.join(res_path, f"preds_{cfg.model['_name']}.csv"), 'a')
    f.write('time,cell,drug1,drug2,pred,label\n')
    for i in range(len(predic)):
        f.write(f'{times[i]},{cell[i]},{pair[i][0]},{pair[i][1]},{predic[i]},{y_test[i]}\n')
    f.close()
    print('save to path: ', os.path.join(res_path, f"preds_{cfg.model['_name']}.csv"))
