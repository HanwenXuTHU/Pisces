...
from .heads_classify_tpm import HeadsClassifyTPM
...
class MMModelConfig(FairseqDataclass):
    ...
    detach_encoder: bool = field(default=True)
...
@register_model("backbone_transfer_model", dataclass=MMModelConfig)
class Pisces_MM_Model(BaseFairseqModel):
    ...
    def forward(self,
        ...
        x = self.classification_heads[classification_head_name](seq_enc_a, graph_enc_a, \
                        seq_enc_b, graph_enc_b, model_input, time, pair, labels)
    ...
    def register_classification_head(self, name, num_classes=None, inner_dim=None, **kwargs):
        ...
        elif name == 'heads_classify_tpm':
            self.classification_heads[name] = HeadsClassifyTPM(
                input_dim=getattr(self.encoder, "output_features", self.args.encoder_embed_dim),
                dv_input_dim=getattr(self.dual_view_encoder, "output_features", self.args.gnn_embed_dim),
                inner_dim=inner_dim or self.args.encoder_embed_dim,
                num_classes=num_classes,
        ...
        for k in state_dict.keys():
            ...
            if getattr(self.args, "load_checkpoint_heads", False):
                if head_name not in current_head_names:
                    self.register_classification_head(head_name, 1, inner_dim)

            # delete the WTA heads
            if k == prefix + "classification_heads." + head_name + ".wta_layer.W":
                keys_to_delete.append(k)
            if 'classifier_1' in k:
                keys_to_delete.append(k)
            if 'classifier_2' in k:
                keys_to_delete.append(k)
            if 'unk' in k:
                keys_to_delete.append(k)
            if 'rel' in k:
...
@register_model_architecture("backbone_transfer_model", "transfer_large")
...
@register_model_architecture("backbone_transfer_model", "transfer_base")
def base_architecture(args):
...
@register_model_architecture("backbone_transfer_model", "transfer_base2")
def base_architecture(args):
...
@register_model_architecture("backbone_transfer_model", "transfer_small")
def base_architecture(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 384)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 512)
    args.encoder_layers = getattr(args, "encoder_layers", 3)
...
@register_model_architecture("backbone_transfer_model", "transfer_tiny")
def base_architecture(args):
