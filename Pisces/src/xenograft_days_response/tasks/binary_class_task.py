...
class BinaryClassTask(FairseqTask):
    ...
    def __init__(self, cfg: BinaryClassConfig, data_dictionary, label_dictionary, model_dictionary, drug_id_dictionary, time_dictionary):
        super().__init__(cfg)
        ...
        self.label_reverse_id = {}
        lines = open(os.path.join(cfg.data, 'label', 'dict.txt')).readlines()
        for line in lines:
            line = line.strip().split()
            self.label_reverse_id[int(line[1])] = line[0]
    ...
    def setup_task(cls, cfg: BinaryClassConfig, **kwargs):
        ...
        model_dict = cls.load_dictionary(os.path.join(cfg.data, 'model', 'dict.txt'))
        logger.info(
            "[model] Dictionary {}: {} types.".format(
                os.path.join(cfg.data, "label"), len(model_dict)
            )
        )
        time_dict = cls.load_dictionary(os.path.join(cfg.data, 'time', 'dict.txt'))
        logger.info(
            "[time] Dictionary {}: {} types.".format(
                os.path.join(cfg.data, "time"), len(time_dict)
            )
        )
        ...
        return cls(cfg, data_dict, label_dict, model_dict, drug_id_dict, time_dict)
    ...
    def load_dataset_src_tgt(self, split):
        ...
        prefix_model = self.get_path('model', split)
        model_dataset = make_dataset(prefix_model, impl=dataset_impl)
        assert model_dataset is not None

        dataset.update(
            model=OffsetTokensDataset(
                StripTokenDataset(model_dataset, id_to_strip=self.model_dictionary.eos()),
                offset=-self.model_dictionary.nspecial,
            )
        )
        ...
        prefix_time = os.path.join(self.cfg.data, 'time', split)
        time_dataset = make_dataset(prefix_time, impl=dataset_impl)
        assert time_dataset is not None
        ...
        dataset.update(
            time=OffsetTokensDataset(
                StripTokenDataset(time_dataset, id_to_strip=self.time_dictionary.eos()),
                offset=-self.time_dictionary.nspecial,
            )
        )
    ...
    def get_embs(self, sample, model, criterion):
        model.eval()
        with torch.no_grad():
            embs, preds, targets, clstypes = criterion.forward_embs(model, sample)
        return embs, preds, targets, clstypes
