...
def validate(
    ...
    pair = np.concatenate(pair_list)
    cell = np.concatenate(cell_list)
    times = np.concatenate(time_list)
    
    emb_path = '/'.join(cfg.task.data.split('/')[0: -2])
    fold_i = cfg.task.data.split('/')[-2]
    emb_path = os.path.join(emb_path, f"embs_{cfg.model['_name']}.pkl")
    ...
    for i in range(len(predic)):
        cmb_name1 = f'{cell[i]}_{pair[i][0]}_{pair[i][1]}_{times[i]}_{fold_i}'
        cmb_name2 = f'{cell[i]}_{pair[i][1]}_{pair[i][0]}_{times[i]}_{fold_i}'
        if cmb_name1 not in emb_dict and cmb_name2 not in emb_dict:
            emb_dict[cmb_name1] = collections.OrderedDict()
            emb_dict[cmb_name1]['emb'] = embeddings[i]
            emb_dict[cmb_name1]['pred'] = predic[i]
            emb_dict[cmb_name1]['target'] = y_test[i]
    save_obj(emb_dict, emb_path)
    print(f"Saving {len(emb_dict)} embeddings")
