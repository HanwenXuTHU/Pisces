...
def validate(
    ...
    fold = eval(cfg.task.data.split('/')[-2].split('fold')[1])
    save_dir = os.path.dirname(cfg.checkpoint.restore_file)
    save_dir = '/'.join(save_dir.split('/')[0: -2])
    file_name = os.path.join(save_dir, 'results.csv')
    with open(file_name, 'a') as f:
        #f.write(f"bacc: {bacc}, auc_prc: {auc_prc}, f1_score: {f1_score}, kappa: {kappa}")
        # save in 4 decimal places
        f.write(f"{fold},{cfg.model.out_modal},{bacc:.4f},{auc_prc:.4f},{f1_score:.4f},{kappa:.4f}\n")
    f.close()
