...
class HeadTopKHidDim(nn.Module):
    ...
    def __init__(self,
        ...
        for mod in self.used_modal:
            if 'Side effect' in mod:
                self.sider_encoder = ModEncoder(self._get_modal_dim(mod), hidden_dim, inner_dim, pooler_dropout)
                self.sider_unk_emb = nn.Embedding(self.drug_n, self._get_modal_dim(mod))
            elif 'Drug Sensitivity (NCI60)' in mod:
                self.nci60_encoder = ModEncoder(self._get_modal_dim(mod), hidden_dim, inner_dim, pooler_dropout)
                self.nci60_unk_emb = nn.Embedding(self.drug_n, self._get_modal_dim(mod))
            elif 'Drug Ontology' in mod:
                self.dron_encoder = ModEncoder(self._get_modal_dim(mod), hidden_dim, inner_dim, pooler_dropout)
                self.dron_unk_emb = nn.Embedding(self.drug_n, self._get_modal_dim(mod))
            elif 'Text' in mod:
                self.text_encoder = ModEncoder(self._get_modal_dim(mod), hidden_dim, inner_dim, pooler_dropout)
            elif '3D' in mod:
                self.drug_3d_encoder = ModEncoder(self._get_modal_dim(mod), hidden_dim, inner_dim, pooler_dropout)
