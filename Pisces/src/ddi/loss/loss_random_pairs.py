...
@register_criterion("loss_random_pairs", dataclass=BinaryClassConfig)
    ...
    def build_input(self, sample, classification_head_name, neg=False):
        if not neg:
            return {
                ...
                'drug_a_seq_random': sample['drug_a_seq_random'] if 'drug_a_seq_random' in  sample else None,
                'drug_b_seq_random': sample['drug_b_seq_random'] if 'drug_b_seq_random' in  sample else None,
                'drug_a_graph_random': sample['drug_a_graph_random'] \
                    if "drug_a_graph_random" in sample else None,
                'drug_b_graph_random': sample['drug_b_graph_random'] \
                    if "drug_b_graph_random" in sample else None,
                'randompair': sample['randompair'] if 'randompair' in sample else None,
                }
        else:
            return {
