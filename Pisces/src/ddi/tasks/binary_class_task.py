...
class BinaryClassTask(FairseqTask):
    ...
    def __init__(self, cfg: BinaryClassConfig, data_dictionary, label_dictionary, drug_id_dictionary):
        super().__init__(cfg)
        self.dictionary = data_dictionary
        self.dictionary.add_symbol("[MASK]")
        self.label_dictionary = label_dictionary
        self.drug_id_dictionary = drug_id_dictionary
    ...
    def setup_task(cls, cfg: BinaryClassConfig, **kwargs):
        ...
        drug_id_dict = cls.load_dictionary(os.path.join(cfg.data, 'pair', 'dict.txt'))
        logger.info(
            "Multi modalities[name] Dictionary {}: {} types.".format(
                os.path.join(cfg.data, "drug_id"), len(drug_id_dict)
            )
        )
        return cls(cfg, data_dict, label_dict, drug_id_dict)
    ...
    def load_dataset_src_tgt(self, split):
        ...
        # load drug neg a and neg b (negtivate sample) ##START##
        drug_a_neg_prefix = self.get_path("", '{}.nega-negb.nega'.format(split))
        drug_a_neg_dataset = make_dataset(drug_a_neg_prefix, impl=dataset_impl)
        drug_b_neg_prefix = self.get_path("", '{}.nega-negb.negb'.format(split))
        drug_b_neg_dataset = make_dataset(drug_b_neg_prefix, impl=dataset_impl)

        drug_a_neg_dataset = AppendTokenDataset(
            TruncateDataset(
                StripTokenDatasetSizes(drug_a_neg_dataset, self.source_dictionary.eos()),
                self._max_positions - 1,
            ),
            self.source_dictionary.eos(),
        )
        drug_b_neg_dataset = AppendTokenDataset(
            TruncateDataset(
                StripTokenDatasetSizes(drug_b_neg_dataset, self.target_dictionary.eos()),
                self._max_positions - 1,
            ),
            self.target_dictionary.eos(),
        )

        drug_a_neg_graph = make_graph_dataset(drug_a_neg_prefix, impl=dataset_impl)
        assert drug_a_neg_graph is not None
        drug_a_neg_graph = Tensor2Data(drug_a_neg_graph)

        drug_b_neg_graph = make_graph_dataset(drug_b_neg_prefix, impl=dataset_impl)
        assert drug_b_neg_graph is not None
        drug_b_neg_graph = Tensor2Data(drug_b_neg_graph)
        ...
        dataset = {
            ...
            "drug_a_seq_neg": {
                "src_tokens": LeftPadDataset(drug_a_neg_dataset, pad_idx=self.source_dictionary.pad()),
                "src_lengths": NumelDataset(drug_a_neg_dataset),
            },
            "drug_b_seq_neg": {
                "src_tokens": LeftPadDataset(drug_b_neg_dataset, pad_idx=self.target_dictionary.pad()),
                "src_lengths": NumelDataset(drug_b_neg_dataset),
            },
            "drug_a_graph_neg": {"graph": drug_a_neg_graph,},
            "drug_b_graph_neg": {"graph": drug_b_neg_graph,},
        }
        ...
        prefix_pair = os.path.join(self.cfg.data, 'pair', split)
        pair_dataset = make_dataset(prefix_pair, impl=dataset_impl)
        assert pair_dataset is not None

        dataset.update(
            pair=OffsetTokensDataset(
                StripTokenDataset(pair_dataset, id_to_strip=self.drug_id_dictionary.eos()),
                offset=-self.drug_id_dictionary.nspecial,
            )
        )

        prefix_negpair = os.path.join(self.cfg.data, 'negpair', split)
        negpair_dataset = make_dataset(prefix_negpair, impl=dataset_impl)
        assert negpair_dataset is not None

        dataset.update(
            negpair=OffsetTokensDataset(
                StripTokenDataset(negpair_dataset, id_to_strip=self.drug_id_dictionary.eos()),
                offset=-self.drug_id_dictionary.nspecial,
            )
        )
