...
class HeadAblationInd(nn.Module):
    ...
    def forward(self, drug_a, dv_drug_a, drug_b, dv_drug_b, net_rel, pair=None, 
        ...
        if np.random.rand() < self.cst_r:
            ...
            _, random_combo_emb = self.get_pred_batch(ra_exp_out, rb_exp_out, rels)
            ...
            random_consine = self.get_cosine_loss(random_xc_1, random_xc_2)
            return out.reshape(-1, 1), consine + random_consine, 0.5 * (sub_out_1 + sub_out_2).reshape(-1, 1)
        else:
