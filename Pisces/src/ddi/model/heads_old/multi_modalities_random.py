...
from .heads_random_pairs import HeadRandomPair
from .heads_topk import HeadTopK
...
class MMModelConfig(FairseqDataclass):
    ...
    rdm_r: float = field(default=0.5)
    is_bitop: bool = field(default=False)
...
@register_model("pisces_random_pairs", dataclass=MMModelConfig)
class Pisces_MM_Model(BaseFairseqModel):
        ...
        with open(drug_name_dict_path, 'r') as f:
            for line in f:
                ...
                try:
                    ...
                    drug_name_dict[line[0]] = eval(line[-1])
    ...
    def forward(self,
                ...
                labels=None,
                drug_a_seq_random=None,
                drug_b_seq_random=None,
                drug_a_graph_random=None,
                drug_b_graph_random=None,
                randompair=None,
                **kwargs):
        ...
        graph_random_a = None
        graph_random_b = None 
        if drug_a_seq_random is not None:
            if np.random.rand() < self.args.rdm_r:
                seq_random_a, _ = self.encoder(**drug_a_seq_random, features_only=features_only, **kwargs)
                seq_random_b, _ = self.encoder(**drug_b_seq_random, features_only=features_only, **kwargs)
                seq_random_a = self.get_cls(seq_random_a)
                seq_random_b = self.get_cls(seq_random_b)

                graph_random_a, _ = self.dual_view_encoder(**drug_a_graph_random, features_only=features_only, **kwargs)
                graph_random_b, _ = self.dual_view_encoder(**drug_b_graph_random, features_only=features_only, **kwargs)
                graph_random_a = self.get_cls(graph_random_a)
                graph_random_b = self.get_cls(graph_random_b)
        ...
        if self.ft_grad_scale:
            ...
            if isinstance(seq_random_a, Tensor):
                seq_random_a = GradMultiply.apply(seq_random_a, 0.1)
            if isinstance(seq_random_b, Tensor):
                seq_random_b = GradMultiply.apply(seq_random_b, 0.1)
            if isinstance(graph_random_a, Tensor):
                graph_random_a = GradMultiply.apply(graph_random_a, 0.1)
            if isinstance(graph_random_b, Tensor):
                graph_random_b = GradMultiply.apply(graph_random_b, 0.1)

        if drug_a_seq_random is not None:   
            x = self.classification_heads[classification_head_name](seq_enc_a, graph_enc_a, \
                        seq_enc_b, graph_enc_b, net_rel, pair, \
                        seq_random_a, graph_random_a, seq_random_b, graph_random_b, randompair,
                        labels)
        else:
            x = self.classification_heads[classification_head_name](seq_enc_a, graph_enc_a, \
    ...
    def register_classification_head(self, name, num_classes=None, inner_dim=None, **kwargs):
        ...
        if name == 'heads_topk':
                self.classification_heads[name] = HeadTopK(
                ...
                seen_drugs=self.seen_drugs,
                topk=self.args.top_k,
                raw_data_path=self.args.raw_data_path,
                drug_target_path=self.args.drug_target_path,
                mix=self.args.mix,
                is_bitop=self.args.is_bitop,
                cst_r=self.args.cst_r,
            )
        elif name == 'heads_random':
            self.classification_heads[name] = HeadRandomPair(
                input_dim=getattr(self.encoder, "output_features", self.args.encoder_embed_dim),
                dv_input_dim=getattr(self.dual_view_encoder, "output_features", self.args.gnn_embed_dim),
                inner_dim=inner_dim or self.args.encoder_embed_dim,
                num_classes=num_classes,
...
@register_model_architecture("pisces_random_pairs", "pisces_large_rdm")
...
@register_model_architecture("pisces_random_pairs", "pisces_base_rdm")
def base_architecture(args):
...
@register_model_architecture("pisces_random_pairs", "pisces_small_rdm")
def base_architecture(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 384)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 512)
    args.encoder_layers = getattr(args, "encoder_layers", 3)
