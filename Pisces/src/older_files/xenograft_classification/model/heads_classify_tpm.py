...
class HeadsClassifyTPM(nn.Module):
    ...
    def __init__(self,
        ...
        # load fpkm vector
        self.fpkm_path = 'data/xenograft/model_fpkm.csv'
        fpkm_csv = pd.read_csv(self.fpkm_path)
        model_names = fpkm_csv['model_names']
        MODEL_TO_INDEX_DICT = {model_names[idx]: idx for idx in range(len(model_names))}
        self.fpkm_vec = collections.OrderedDict()
        for i in range(fpkm_csv.shape[0]):
            m_name, vec = fpkm_csv.iloc[i, 0], fpkm_csv.iloc[i, 1:].values
            m_id = MODEL_TO_INDEX_DICT[m_name]
            self.fpkm_vec[m_id] = np.log(np.asarray(vec, dtype=np.float32).reshape(-1)+1)
        self.model_size = len(vec)
        ...
        self.model_neighbor_set = ppi_loader.get_model_neighbor_set()
        ...
        self.layernorm_model = nn.LayerNorm(inner_dim)
        self.model_encoder = nn.Sequential(
            nn.Linear(self.model_size, 512),
            nn.ReLU(),
            nn.Linear(512, 256),
            nn.ReLU(),
        ...
        self.classifier_2 = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(p=pooler_dropout),
            nn.Linear(inner_dim, self.num_classes),
            nn.Softmax(dim=-1)
        )
    ...
    def get_pred_batch(self, a, b, models):
        if len(a.shape) == 2:
            a, b = a.unsqueeze(1), b.unsqueeze(1)
        models_batch = models.repeat(1, a.shape[1], 1)
        x = self.classifier_1(torch.cat([a, b, models_batch], dim=-1))
    ...
    def get_model_embeddings(self, models):
        models_npy = models.squeeze().cpu().numpy()
        model_emb = [self.fpkm_vec[models_npy[i]].reshape([1, -1]) for i in range(models_npy.shape[0])]
        model_emb = torch.from_numpy(np.concatenate(model_emb, axis=0)).to(models.device)
        if self.fp16:
            model_emb = model_emb.half()
        return model_emb

    def forward(self, drug_a, dv_drug_a, drug_b, dv_drug_b, models, pair=None, labels=None):

        model_emb = self.get_model_embeddings(models)
        # model_emb = F.normalize(model_emb, 2, -1)
        model_embeddings = self.model_encoder(model_emb)
        model_embeddings = model_embeddings.unsqueeze(1)
        ...
        pred, all_combo_emb = self.get_pred_batch(a_exp_i, b_exp_i, model_embeddings)
