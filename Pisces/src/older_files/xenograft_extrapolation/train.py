...
def validate(
    ...
    for subset in subsets:
        ...
        # pdb.set_trace()
        pearson = np.corrcoef(preds_val, labels_val)[0, 1]
        spr = spearmanr(preds_val, labels_val)
        rmse = np.sqrt(mean_squared_error(preds_val, labels_val))
        
        stats = agg.get_smoothed_values()
        stats['pearsonr'] = pearson
        stats['spearmanr'] = spr
        stats['rmse'] = rmse
        stats['combined'] = 0.5*stats['pearsonr'] + 0.5*stats['spearmanr'] - stats['rmse']
