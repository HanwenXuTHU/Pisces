...
import pandas as pd
import pdb
from .heads_ppi import DDIDataPPI
from .heads_wta import Heads_WTA


def get_neighbor_set(n_memory, drug_target_path, drug_dict, n_hop):
    ppi_loader = DDIDataPPI(
    ...
    return drug_neighbor_set, protein_num
...
class HeadsClassify(nn.Module):
    ...
    def __init__(self,
        ...
        self.drug_neighbor_set, self.protein_num = get_neighbor_set(self.n_memory, drug_target_path, self.drug_dict, self.n_hop)
    ...
    def set_nn(self, input_dim, dv_input_dim, inner_dim, pooler_dropout, wta_linear, drug_n):
        ...
        self.wta_layer = Heads_WTA(len(self.out_modal)**2, topk=self.topk, is_linear=wta_linear, is_mask=True)
    ...
    def forward(self, drug_a, dv_drug_a, drug_b, dv_drug_b, net_rel, pair=None, 
        ...
        a_exp_out, b_exp_out, a_exp_mask, b_exp_mask = self.expand_ab(a_i_out, b_i_out, a_mask, b_mask)
    ...
    def _get_pair_input(self, pair):
        ...
        for mod in self.out_modal:
            ...
            if mod in ['SMILES', 'Graph']:
                continue
            f_a, f_b = [], []
            for s, i in enumerate(np_pairs[:, 0]):
                ...
                if self.raw_data[d_a_name][mod] is not None:
                    ...
                    if self.fp16: a_i = a_i.half()
                    f_a.append(a_i)
            for s, i in enumerate(np_pairs[:, 1]):
