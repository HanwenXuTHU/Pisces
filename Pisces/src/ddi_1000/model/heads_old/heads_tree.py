...
class MultiHeadTree(nn.Module):
    ...
    def __init__(self,
        ...
        self.num_tree_level = len(self.used_modal)
        ...
        for i in range(len(self.used_modal)):
            self.classifier_1_list.append(nn.Sequential(
                nn.Linear(2 * inner_dim * (i + 1), inner_dim),
                nn.Tanh(), 
                nn.Dropout(p=pooler_dropout),
            ))
    ...
    '''def get_mod_mask(self, a_rank, b_rank, pair):
        ...
        for i in range(a_rank.shape[1]):
            is_modal_a = self.is_modal[np_pair[:, 0], np_a_rank[:, i]]
            is_modal_b = self.is_modal[np_pair[:, 1], np_b_rank[:, i]]
            is_seen_a = np.asarray([1 if str(d) in self.seen_drugs else 0 for d in np_pair[:, 0]])
            is_seen_b = np.asarray([1 if str(d) in self.seen_drugs else 0 for d in np_pair[:, 1]])
            mask_a = (is_modal_a + is_seen_a) >= 1
            mask_b = (is_modal_b + is_seen_b) >= 1
            mask_a_list.append(mask_a.reshape(-1, 1))
            mask_b_list.append(mask_b.reshape(-1, 1))
        mask_a = np.concatenate(mask_a_list, axis=1)
        mask_b = np.concatenate(mask_b_list, axis=1)
    ...
    def get_pred_batch(self, a, b, rels, tree_level=0):
        if len(a.shape) == 2:
            a, b = a.unsqueeze(1), b.unsqueeze(1)
        x = self.classifier_1_list[tree_level](torch.cat([a, b], dim=-1))
        return self.x_rels_pred(x, rels)
    
    def x_rels_pred(self, x, rels):
        rels_batch = rels.repeat(1, x.shape[1], 1)
        rel_input = torch.cat([x, rels_batch], dim=-1)
        rels_hat = self.rels_linear(rel_input) + rels_batch
        # dot product along the last dimension
        pred = torch.multiply(rels_hat, x).sum(dim=-1)
        return pred.squeeze(), x.squeeze()

    def get_mod_rank(self, out_a, out_b, rels):
        a_mod_scores = torch.zeros((rels.shape[0], len(self.used_modal))).to(rels.device).half()
        b_mod_scores = torch.zeros((rels.shape[0], len(self.used_modal))).to(rels.device).half()
        for mod1 in self.used_modal:
            for mod2 in self.used_modal:
                pred, _ = self.get_pred_batch(out_a[mod1], out_b[mod2], rels)
    ...
    def forward(self, drug_a, dv_drug_a, drug_b, dv_drug_b, net_rel, pair=None, labels=None):
        ...
        all_mask = []
        a_rank, b_rank = self.get_mod_rank(out_a_dict, out_b_dict, rels)
        ...
        for i in range(self.num_tree_level):
            ...
            pred, x_emb = self.get_pred_batch(a_exp_i, b_exp_i, rels, tree_level=i)
        ...
        out = torch.sum(all_pred * all_mask, dim=1) / torch.sum(all_mask, dim=1)
