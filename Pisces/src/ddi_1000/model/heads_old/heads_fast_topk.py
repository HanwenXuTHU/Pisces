...
class HeadFastTopK(nn.Module):
    ...
    def get_pred_batch(self, a, b, rels):
        ...
        x = self.classifier_1(torch.cat([a, b], dim=-1))
    ...
    def forward(self, drug_a, dv_drug_a, drug_b, dv_drug_b, net_rel, pair=None, labels=None):
        ...
        a_i, b_i = self.dict_to_emb(out_a_dict, out_b_dict)

        a_exp_i, b_exp_i = self.expand_ab(a_i, b_i)
        pred, all_combo_emb = self.get_pred_batch(a_exp_i, b_exp_i, rels)
        if len(pred.shape) == 1: pred, x_emb = pred.unsqueeze(-1), x_emb.unsqueeze(1)

        out, top_idx = torch.topk(pred, k=self.topk, dim=1)
